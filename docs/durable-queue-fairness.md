...
- **Starvation bound.** The bound is on lane service, not on individual jobs: with A non-empty lanes and quantum 1, each lane is served once per scheduling round of at most A jobs, so every lane drains at at least 1/A of worker throughput. A job at position p in its lane therefore starts within (p + 1) * A worker turns. What this rules out is the unbounded case: under today's cross-installation FIFO, a sustained burst from one installation can postpone another installation's job indefinitely (no bound at any load); under DRR every queued job has a finite, computable worst-case wait. Within a lane, a burst still delays the lane's own later jobs - that is the installation delaying itself, which is correct.
- **Ordering.** Each lane is FIFO within its class (below), so per-repository order is preserved exactly as today; supersede (#257) drops the replaced job inside its lane and changes nothing else.
...
- **Bulk scans.** `full_scan` jobs from a fresh installation arrive as a burst (one per repository). Plain FIFO inside a lane would make the installation's own PR jobs queue behind its whole scan, so each lane carries two classes: interactive (`pull_request`, `push`, `rerequested`) and bulk (`full_scan`). The scheduler picks the lane by DRR, then the lane serves its oldest interactive job, else its oldest bulk job. The burst therefore cannot monopolise workers (DRR across lanes) and cannot starve the installation's own PR jobs (class order within the lane); bulk scans get exactly the lane's spare service. Class order is the only intra-lane priority; there is no cross-lane priority tier.
