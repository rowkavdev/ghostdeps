...
## Production bring-up checklist (not a go-live authorization)

This is the sequence **after Rowan decides to go live**. It does not authorize registration, installation, credentials, spending, or a deployment now. Prepare the host and a release commit beforehand if useful; record the chosen app identity and target repository at go-live. Do not assume `/healthz` alone proves the integration works.

1. **Decision point - Rowan: go-live timing and registration.** Get Rowan's explicit go for production registration and the target installation/repository. Until then, stop here. Confirm a host with Node 22, 2 vCPU / 4 GiB as the small-installation starting point, temporary disk headroom, outbound GitHub API/codeload access, and a public HTTPS webhook URL proxied to the single service. Pin the release commit and keep the previous build for rollback.
2. **Decision point - Rowan: app identity under [#221](https://github.com/rowkavdev/ghostdeps/issues/221).** Choose whether the approved production setup uses one existing GitHub App or a second isolated App. In the single-app branch, verify that the chosen App is authorized for this product, matches the manifest's permissions/events, and that its identity and rate budget are not shared with swarm lanes or public ghstats reads. If that cannot be verified, stop rather than reusing its key. In the second-app branch, register a separate GhostDeps product App with only the manifest's scopes/events and keep its private key separate from lane credentials. Do not create/install the #221 automation App or change ghstats credentials as part of this product bring-up. Either choice needs Rowan's decision; neither is an implicit default.
3. **Register/configure the chosen product App.** Use [`packages/github-app/app.yml`](https://github.com/rowkavdev/ghostdeps/blob/main/packages/github-app/app.yml) as the permission/event checklist. Set its webhook URL to `https://YOUR_HOST/api/github/webhooks`, enable webhooks, record the App ID, generate its private key, and set a random webhook secret. Review the actual GitHub permissions/events against [the table](github-app.md) before saving; do not grant broader access. Store the key and secret in the host's secret manager or root-controlled environment file, never in Git, chat, logs, or a command transcript. If reusing an App, verify its webhook settings and key rotation plan rather than silently replacing a working endpoint.
4. **Deploy one service instance.** Set `APP_ID`, `PRIVATE_KEY`, `WEBHOOK_SECRET`, `NODE_ENV=production`, and optionally a short `GHOSTDEPS_RELEASE_ID` from the pinned release tag/SHA. Pick the [container](#container-deployment) or [Node/systemd](#direct-nodesystemd-deployment) path below, bind the Node listener behind TLS, and start it. Do not put a private key or webhook secret in the image. Keep the runtime's worker token scoped per repository (`contents: read`, `checks: write`); do not share it with swarm lanes.
5. **Check liveness, then delivery.** Fetch `https://YOUR_HOST/healthz` through the public proxy and confirm HTTP 200, `Cache-Control: no-store`, and JSON `status: "ok"` with a nonnegative integer `uptimeSeconds`. `version` must equal the deploy-supplied release ID if valid, or be absent if none was supplied. No other fields should appear. This is liveness, not readiness or GitHub authentication. Inspect service logs without printing env values.
6. **Install on the chosen repository and verify the first real run.** Install the chosen App only on Rowan-approved repositories. The `installation.created` event (or `installation_repositories.added` for a later repo) should queue a full scan at that repository's _current default branch head_. In GitHub's App recent deliveries, confirm a signed delivery reached the webhook with HTTP 2xx; then inspect that exact head's `ghostdeps` check run and its completion (`success` for a quiet clean result, `neutral` for findings or an incomplete run). An HTTP 2xx or health response without a completed real check is not enough. If the queue is full, the busy-neutral check asks for a new push; do not call that a successful analysis. For a failed/missing run, inspect the delivery and service logs, fix the cause, and use the check re-run button or a new commit if needed.
7. **Close out or roll back.** Record the App identity, installed repository, release SHA, delivery ID, check-run URL/conclusion, and any limits without recording credentials. Watch logs, API limits, CPU, memory and temporary disk during the first runs. If the service fails, restore the previous release as described below; restarting drops queued in-memory jobs, which must be re-run. Do not mark bring-up complete until a real check run lands.
