...
The [GitHub App](../github-app.md) **does not call the dependency review endpoint**. For same-repo PRs it fetches the `base...head` compare diff under `contents: read`, reads changed `package.json` manifests at both SHAs as text, and uses core's [dependency-change extractor](https://github.com/rowkavdev/ghostdeps/blob/main/packages/core/src/diff/dependency-changes.ts) to scope JS/TS findings. Source-line changes help with annotations. If the diff or manifest read is incomplete on a dependency-changing PR, the App falls back to a full-repository analysis, because narrowing on partial data could hide a finding. When the complete changed-file list shows a source-only PR, an incomplete diff instead leaves it PR-scoped with no dependency changes and a "Removed-usage check skipped" note; a full scan would surface unrelated verdicts. Fork re-runs without a PR link also get full analysis. Other ecosystems' manifest-change coverage is not implied by the current JS/TS PR path.
