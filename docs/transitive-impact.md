...
- Per entry, core sums the sizes of the dependency itself and **every locked version** of each closure member (closures are by name), deduplicated by name and version. `coverage` is `{ sized, total }`. `bytes` is approximate in both directions: it undercounts when `sized < total`, and it can overcount when a closure member has several locked versions, because a dependency is charged for versions only other dependencies pull in (for example, `a -> c` and `b -> c` with `c@1` and `c@2` locked charges `a` for both). A version-aware walk is tracked in #288. An entry with `transitive: null` or no sized package gets no footprint.
