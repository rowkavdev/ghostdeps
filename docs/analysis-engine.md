...
- **Determinism.** Projects, dependencies, usages, findings, detections and surfaces are sorted by stable keys, so adapter order and completion timing never change the output. Findings sort by kind, dependency, first evidence location, summary, affected files, recommendation.
