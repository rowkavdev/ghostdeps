...
7. Same-SHA re-runs (#174): a finished analysis is kept in a bounded in-process cache (16 entries per repository, 256 repositories). The key is the repository, head SHA, base SHA, source-only flag, the core/adapter contract versions and the worker's policy, adapter and scan config. A re-run with the same key reuses it and skips the download and analysis, so its output equals the first run's. Only clean analyses are kept. Anything with an adapter error or an app note (skipped step, full-repository fallback) is re-analysed, because a re-run is how users retry those. The cache is empty after a restart. The worker option `resultCache: false` turns it off.
