...
5. Run `analyseRepositoryIsolated` with the JS/TS adapter in a worker thread (#112). If the checkout scan was truncated or skipped paths, the worker sets `scanIncomplete`, so the policy never calls a dependency "unused" on a partial checkout (#136).
